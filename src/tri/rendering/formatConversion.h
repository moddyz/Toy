...
{
    static inline bool Execute( size_t i_numPixels, const gm::Vec3f* i_pixelBuffer, uint32_t* o_pixelBuffer )
    {
        for ( size_t pixelIndex = 0; pixelIndex < i_numPixels; ++pixelIndex )
        {
            const gm::Vec3f& inPixel  = i_pixelBuffer[ pixelIndex ];
            uint8_t*         outPixel = reinterpret_cast< uint8_t* >( &o_pixelBuffer[ pixelIndex ] );
...
__global__ void
ConvertRGBFloatToRGBAUint32_Kernel( size_t i_numPixels, const gm::Vec3f* i_pixelBuffer, uint32_t* o_points );
...
struct ConvertRGBFloatToRGBAUint32< CUDA >
{
    static inline bool Execute( size_t i_numPixels, const gm::Vec3f* i_pixelBuffer, uint32_t* o_pixelBuffer )
    {
        ...
        // Arguments
        std::array< void*, 3 > args = {&i_numPixels, &i_pixelBuffer, &o_pixelBuffer};
