...
protected:
    ...
    inline virtual void OnResize(const gm::Vec2i& i_dimensions) { /* no-op */ }

    /// Respond to a key press event.
    ///
    /// \param i_key The ID of the key which was pressed.
    /// \param i_action ???
    /// \param i_modifiers If any keyboard modifiers are active (ctrl, alt,
    /// shift).
    ...
    inline virtual void OnMouseMove(const gm::Vec2f& i_position) { /* no-op */ }
    ...
    /// \param i_modifiers If any keyboard modifiers are active (ctrl, alt,
    /// shift).
    inline virtual void OnMouseButton(int i_button,
                                      int i_action,
                                      int i_modifiers)
    {
        /* no-op */
    ...
    inline virtual void OnScroll(const gm::Vec2f& i_offset) { /* no-op */ }
...
private:
    ...
    static void _KeyCallback(GLFWwindow* i_glfwWindow,
                             int i_key,
                             int i_scanCode,
                             int i_action,
                             int i_modifiers);
    static void _MouseMoveCallback(GLFWwindow* i_glfwWindow,
                                   double i_xCoord,
                                   double i_yCoord);
    static void _MouseButtonCallback(GLFWwindow* i_glfwWindow,
                                     int i_button,
                                     int i_action,
                                     int i_modifiers);
    static void _ScrollCallback(GLFWwindow* i_glfwWindow,
                                double i_xOffset,
                                double i_yOffset);
    static void _FrameBufferSizeCallback(GLFWwindow* i_glfwWindow,
                                         int i_width,
                                         int i_height);
