...
{
public:
    inline explicit OrbitManipulator(LookAtTransform& o_transform,
    ...
    {
        // Compute the coordinates of the camera origin, with the target as the
        // coordinate system origin.
        gm::Vec3f targetCentricOrigin =
            m_transform.GetOrigin() - m_transform.GetTarget();
        ...
        gm::Mat4f rotationTransform =
            gm::MatrixProduct(pitchTransform, yawTransform);

        // Perform transformation in target-space, then bring back into
        // world-space.
        gm::Vec3f newTargetCentricOrigin =
            gm::TransformPoint(rotationTransform, targetCentricOrigin);
        ...
        if (gm::DotProduct(gm::Normalize(m_transform.GetTarget() - newOrigin),
                           newUp) > 0.99f) {
            newUp = m_transform.GetNewUp();
        }

        m_transform = tri::LookAtTransform(
            newOrigin, m_transform.GetTarget(), m_transform.GetNewUp());
    }
