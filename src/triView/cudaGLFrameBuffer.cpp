...
CUDAGLFrameBuffer::CUDAGLFrameBuffer(int i_width, int i_height)
  ...
  , m_height(i_height)
{
    printf("Creating frame buffer with dimensions (w=%i, h=%i).\n",
           m_width,
           m_height);
    ...
    CUDA_CHECK(cudaGraphicsGLRegisterBuffer(&m_graphicsResource,
                                            m_pixelBufferId,
    ...
    TRI_VERIFY(glGetError() == GL_NO_ERROR);
}

CUDAGLFrameBuffer::~CUDAGLFrameBuffer()
{
    printf("Destroying frame buffer.\n");
...
uint32_t*
CUDAGLFrameBuffer::Map()
{
    CUDA_CHECK(cudaGraphicsMapResources(1, &m_graphicsResource, 0));
    uint32_t* devicePtr = nullptr;
    size_t numBytes;
    CUDA_CHECK(cudaGraphicsResourceGetMappedPointer(
        (void**)&devicePtr, &numBytes, m_graphicsResource));
    TRI_VERIFY(numBytes == GetByteSize());
    return devicePtr;
}

void
CUDAGLFrameBuffer::Unmap()
{
...
CUDAGLFrameBuffer::Draw()
{
    ...
    // indexes into a 2D array, as if the (0, 0) coordinate is at the top-left
    // corner.
    glBegin(GL_QUADS);
    glTexCoord2f(0, -1.0f);
    glVertex3f(0, 0, 0);
